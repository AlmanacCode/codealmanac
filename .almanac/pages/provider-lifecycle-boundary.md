...
`src/commands/setup.ts` was the next provider-adjacent cohesion risk. It orchestrated global install handling, provider/model selection, instruction-guide installation, automation setup, config writes, prompt/output flow, and path resolution. The cleanup kept `setup.ts` as the CLI wrapper and moved named workflow steps into `src/commands/setup/` modules such as provider/model choice, guide installation, durable install-path handling, next-step rendering, and terminal output. That preserves setup as one user-facing command while preventing setup from becoming the owner of reusable provider readiness, config, guide, and automation behavior.
