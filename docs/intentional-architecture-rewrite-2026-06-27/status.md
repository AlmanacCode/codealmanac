...
The branch has more than 280 committed rewrite commits past `dev`. The worklog records 278 production slices so far.
...
The latest slice split show CLI output formatting by rendering responsibility. `show/format.ts` now owns output-mode routing, while `show/body.ts`, `show/fields.ts`, `show/metadata.ts`, and `show/time.ts` own the specific formatting details that were previously mixed into the 275-line formatter.

Verification passed:

- `npm run lint`
- `npx vitest run test/architecture-wiki-command-boundaries.test.ts test/show.test.ts`
