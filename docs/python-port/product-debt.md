...
| Agent harness contract | partially resolved: lifecycle workflows record returned harness status/output before validation. Re-evaluate old Codex app-server and Claude SDK adapters only if local lifecycle dogfood needs streaming, structured output, usage, subagents, or richer transcript controls. |
