...
| `src/edges/cli/` | Commander registration, process argv handling, sqlite-free recovery path, CLI-edge event rendering, terminal setup UI, and local CLI process lifetimes | `register-commands.ts`, `register-query-commands.ts`, `register-edit-commands.ts`, `register-review-commands.ts`, `register-page-topic-commands.ts`, `register-migrate-commands.ts`, `register-topics-commands.ts`, `register-lifecycle-run-commands.ts`, `register-sync-commands.ts`, `register-jobs-commands.ts`, `register-automation-commands.ts`, `register-setup-commands.ts`, `setup/`, `serve.ts` |
...
| `src/services/setup/` | Setup and uninstall product workflows over agent readiness/config state, provider fix commands, global install state/execution, instruction targets, automation cleanup, and guide cleanup while CLI edge setup files own terminal UI | `agent-choice.ts`, `provider-fix-command.ts`, `global-install.ts`, `auto-commit.ts`, `instructions.ts`, `uninstall.ts`, `index.ts` |
