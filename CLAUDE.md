...
| `src/shared/` | Small cross-cutting helpers and neutral contracts with no product workflow ownership | `duration.ts`, `agent-provider.ts`, `agent-runtime/`, `diagnostics.ts`, `pid-liveness.ts`, `transcripts.ts`, `worker-program.ts` |
