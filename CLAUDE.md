...
Intelligence lives in prompts, not pipelines. When judgment is needed — deciding what a session produced, scoring notability, evaluating a proposed page against the graph — we hand a concrete-but-open prompt to an agent. We do not wrap agents in propose/review/apply state machines, intermediate proposal files, or `--dry-run` rehearsals. The writer owns outcomes and calls the reviewer as a subagent when it wants feedback; there is no orchestration JSON schema between them. Everything is **local-only** (`.almanac/` per repo, `~/.almanac/registry.json` globally, no hosted service), the `.almanac/` namespace is **flat** (no `.almanac/wiki/` subdir — future features get peer files), and **only lifecycle commands touch AI or write wiki pages.** Query, organization, status, auth, and maintenance commands stay deterministic over local state, the filesystem, and network services they explicitly configure.
...
- **Query and organization commands never read or write page content.** Lifecycle commands (`init`, `capture`, `ingest`, and `garden`) may hand bounded source context to an agent and write wiki pages. Every other command operates on `index.db`, frontmatter, connector state, and the filesystem only.
